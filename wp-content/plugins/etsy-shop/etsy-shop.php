...
/*
 * Copyright 2011-2022  Frédéric Sheedy
 *
...
define( 'ETSY_SHOP_VERSION',  '3.0.2' );
...
function etsy_shop_process() {
    ...
    switch ($num_args) {
        case (1):
            ...
            $show_available_tag = ( !$attributes['show_available_tag'] ? false : wp_strip_all_tags( $attributes['show_available_tag'] ) );
            $language           = ( !$attributes['language'] ? null : wp_strip_all_tags( $attributes['language']) );
            $columns            = ( !$attributes['columns'] ? 3 : wp_strip_all_tags( $attributes['columns'] ) );
